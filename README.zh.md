...
6. 每次成功跑完后，工作流会把当次更新后的 `docs/`（各期 HTML、`index.html`、`.index.json`、CSS）**提交**回 **`main` / `tomatonewsdaily` 分支**，这样 **历史邮件里的「打开日报」链接在 GitHub Pages 上仍有效**（每次部署仍包含完整 `docs/` 树）。若 **分支保护** 禁止直推，需为 **`github-actions[bot]`** 设例外或放宽规则，否则该步骤会失败。
...
docs/              # 站点；各期 HTML 与归档在 git 中由 CI 提交，保证链接长期有效
